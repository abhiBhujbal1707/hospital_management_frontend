...
//         reset();
//         setSelectedTime("");
//         fetchAppointments(); // Refresh the list after adding a new appointment
//       } else {
//         setMessage({ type: "error", text: result.message || "Failed to book appointment." });
//       }
//     } catch (error) {
...
//     try {
//       const response = await fetch(`http://localhost:5116/api/appointments/${id}`, {
//         method: "DELETE",
//       });
...
//         <div>
//           <label className="block text-sm font-medium text-gray-700 mb-1">First Name</label>
//           <input type="text" {...register("fname", { required: "First name is required" })} className="w-full h-12 pl-2 bg-zinc-100 border-gray-300 rounded-lg shadow-sm" />
//           {errors.fname && <p className="text-sm text-red-600 mt-1">{errors.fname.message}</p>}
//         </div>

//         <div>
//           <label className="block text-sm font-medium text-gray-700 mb-1">Doctor Name</label>
//           <input type="text" {...register("drname", { required: "Doctor Name is required" })} className="w-full h-12 pl-2 bg-zinc-100 border-gray-300 rounded-lg shadow-sm" />
//           {errors.drname && <p className="text-sm text-red-600 mt-1">{errors.drname.message}</p>}
//         </div>

//         <div>
//           <label className="block text-sm font-medium text-gray-700 mb-1">Time</label>
//           <input type="time" value={selectedTime} readOnly className="w-full h-12 pl-2 bg-zinc-100 border-gray-300 rounded-lg shadow-sm" />
//           <TimeSlots onSelectTime={handleTimeSelect} />
//           {errors.time && <p className="text-sm text-red-600 mt-1">{errors.time.message}</p>}
//         </div>

//         <button type="submit" disabled={loading} className="bg-blue-500 text-white px-6 py-2 rounded-lg shadow hover:bg-blue-600 focus:outline-none">
//           {loading ? "Submitting..." : "Submit"}
//         </button>
//       </form>
...
//           <li key={appt.id} className="flex justify-between bg-white shadow-sm p-3 rounded-lg mb-2">
//             <span>{appt.firstName} {appt.lastName} - {appt.time}</span>
//             <button onClick={() => deleteAppointment(appt.id)} className="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-700">
//               Delete
//             </button>
//           </li>
//         ))}
